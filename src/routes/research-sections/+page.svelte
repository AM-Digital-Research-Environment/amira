...
<EntityPageContainer
	title="Research Sections"
	subtitle="The Cluster's research is organised into Research Sections, providing a coherent structure for inter- and transdisciplinary collaboration between researchers from Bayreuth, Africa, and a global network. Africa Multiple 1.0 ran six Sections from 2019 to 2025; Africa Multiple 2.0 pursues research in six new Sections starting in June 2026."
	selected={() => selectedSectionData}
>
	{#snippet detailView()}
		<EntityDetailViewShell
			backLabel="Back to all sections"
			onBack={clearSelection}
			resolved={selectedSectionData}
			loading={detail.loading}
			emptyMessage="No data available for this section."
		>
			{#snippet body()}
				<!-- The shell only renders this body when `resolved` is truthy,
				     but svelte-check needs an explicit narrow to read
				     `selectedSectionData.foo` below. The inner `{#if}` is
				     redundant at runtime — the shell already gates it. -->
				{#if selectedSectionData}
					<!-- Section header -->
					<Card
								...
								{#snippet children()}
									<div class="flex items-start justify-between gap-3 min-w-0">
										...
										<div class="flex items-center gap-3 shrink-0">
											...
											{/if}
											<WissKILink
												category="researchSections"
												entityKey={selectedSectionData.name}
											/>
